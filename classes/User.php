<?php
    # Note: The User.php is going to hold the logic of the application like the CRUD (Create, Read, Update, Delete),
    # data manipulation, arithmetic operations

    // include "Database.php"; # not strict
    ...
    class User extends Database {
        ...
        public function login($request) {
            ...
            # Query string to chech the username if it exists in the database
            ...
            if ($result->num_rows == 1) { 
                ...
                # $user = ['id' => 1, 'first_name' => 'Mary', 'last_name' => 'Watson', 'username' => 'mary12345', 'password' => $2y$10$my.eVsTnGUpmLO2wrCW....]
                
                # check / verify if the password supplied by the user from the login form is the same or equal to the password already in the database
                if (password_verify($password, $user['password'])) {
                    # If the password is matched and verified
                    # Create a SESSION VSRIABLES for future use 
                    # we can retrieve the session data inside "The-Company" folder
                    session_start(); # start the session
                    $_SESSION['id'] = $user['id']; # <= 1
                    $_SESSION['username'] = $user['username']; # <= mary12345
                    $_SESSION['fullname'] = $user['first_name']." ".$user['last_name']; # <= Mary Watson
